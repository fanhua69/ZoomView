						   ...
						   UINT nFirstChar)
{
	...
	if (!Create(dwComboStyle, rect, pParent, IDC_IPLIST)) return;
...
int CInPlaceList::GetRow()
{
    return m_nRow;
}

int CInPlaceList::GetCol()
{
    return m_nCol;
}
...
BEGIN_MESSAGE_MAP(CInPlaceList, CComboBox)
	...
	ON_WM_KEYDOWN()
	ON_WM_KEYUP()
    ON_EN_CHANGE(IDC_COMBOEDIT,EditModified)
...
void CInPlaceList::EditModified()
{
    CString str;
    GetWindowText(str);
    ...
    dispinfo.hdr.hwndFrom = GetSafeHwnd();
    dispinfo.hdr.idFrom   = GetDlgCtrlID();
    dispinfo.hdr.code     = GVN_ITEMCHANGED;
