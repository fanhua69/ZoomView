...
BEGIN_MESSAGE_MAP(CSortedListCtrl, CListCtrlEx)
...
void CSortedListCtrl::SortColumn (int nCol, BOOL bAsc)
{
    if (m_Sorting) {
        ...
        // Now sort all items in column using CListCtrlEx sorting mechanism
...
int CSortedListCtrl::CompareItems(CItemInfo *pItemInfo1, CItemInfo *pItemInfo2)
{
    ...
    CString sValue1=pItemInfo1->GetColValue(iCol);
    CString sValue2=pItemInfo2->GetColValue(iCol);
    double dValue1,dValue2;
    if(pItemInfo1->IsNumber(iCol))
    {
        sscanf((LPCTSTR)sValue1,"%lf",&dValue1);
        sscanf((LPCTSTR)sValue2,"%lf",&dValue2);
        return dValue1>dValue2;
    }

    else
    {
        return sValue1.CompareNoCase(sValue2);
    }
}
...
BOOL CSortedListCtrl::SetGridLines(BOOL bGridLine)
{
    // no painting during change
    LockWindowUpdate();
...
BOOL CSortedListCtrl::GetGridLine()
{
    return(m_bGridLine);
}
...
{
    CListCtrlEx::OnSetFocus(pOldWnd);
...
void CSortedListCtrl::OnGetDispInfo(NMHDR* pNMHDR, LRESULT* pResult) 
{
    ...
    if (pDispInfo->item.mask & LVIF_TEXT) 
    {
        ...
        CItemInfo* pAppItem = reinterpret_cast<CItemInfo*>(pDispInfo->item.lParam);
        if(pAppItem )
        {
            lstrcpy (pDispInfo->item.pszText, pAppItem->GetColValue(pDispInfo->item.iSubItem));
        }
    }
    *pResult = 0;
}
