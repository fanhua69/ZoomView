...
//                                 Fixec typo (my fault!) in CMemDCSimple.h - Claus Arend-Schneider)
...
//          1.04    6 April 1998   Added Ctrl-A = Select All, fixed CGridDropTarget 
...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
	...
	m_pMyParent          = NULL;	// Added By FanHua
	m_bShiftSelecting	 = FALSE;	// Added By FanHua

	m_bCellPicKeepRatio = false;
	m_iCellPicWidth		 = 100;
	m_iCellPicHeight     = 80;
	m_bDrawCellPic		 = true; 
...
{    
    ...
    // m_DropTarget.Register(this);     // <-- causes problems in win95
...
BEGIN_MESSAGE_MAP(CGridCtrl, CWnd)
    ...
    ON_CBN_SELENDOK     (IDC_IPLIST,OnSelOKInCombo)
    ON_NOTIFY(GVN_ITEMCHANGED, IDC_IPLIST, OnComboChanged)

    ON_NOTIFY(GVN_ENDLABELEDIT, IDC_IPEDIT, OnEndInPlaceEdit)
    ON_NOTIFY(LVN_ENDLABELEDIT, IDC_IPLIST, OnEndInPlaceEdit)
...
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
    ...
    else
    {
        switch (nChar)
        {
            case VK_DELETE: SetItemText(m_idCurrentCell.row, m_idCurrentCell.col, _T(""));
							...
							if(m_pMyParent)
							{
								m_pMyParent->PostMessage(GV_MODIFIED,
														m_idCurrentCell.row,
														m_idCurrentCell.col);
							}
	...
	if(IsSHIFTpressed())
	{
		if(!m_bShiftSelecting)
		{
			m_bShiftSelecting=true;
...
{    
	...
	if(   pgvItem->row<0||pgvItem->row>=m_nRows
		||pgvItem->col<0||pgvItem->col>=m_nCols)
	{
		return;
	}
...
void CGridCtrl::OnDraw(CDC* pDC)
{
	...
	if(m_bDrawCellPic)
	{
		...
		int nPics=m_arrayPics.GetCount ();		
		for(int i=0;i<nPics;i++)
		{
			CFHImage image;
			pic=m_arrayPics.GetAt (i);
			HRESULT h=image.Load (pic.sPath );
			if(h!=S_OK)
			{
				continue;
			}
			...
			if(!GetCellRect(pic.iRow ,pic.iCol ,rect))
			{			
				continue;
			};

			rect.DeflateRect (2,2,3,2);

			image.Show (pDC->m_hDC ,rect,m_bCellPicKeepRatio);
		}
	}
...
void CGridCtrl::CutSelectedText()
{
    ...
    for (POSITION pos = m_SelectedCellMap.GetStartPosition(); pos != NULL; )
    {
		...
		if(m_pMyParent)
		{
			m_pMyParent->PostMessage(GV_MODIFIED,cell.row ,cell.col ); //
		}
...
void CGridCtrl::SetColEditable(bool bEditable, int iColStart, int iColEnd)
{
    SetEditable(bEditable,-1,iColStart, -1,iColEnd);
}

void CGridCtrl::SetRowEditable(bool bEditable, int iRowStart, int iRowEnd)
{
    SetEditable(bEditable,iRowStart,-1,iRowEnd, -1);
}
                            ...
                            int     iColEnd)
{
    ...
    if(iRowEnd==-1||iRowEnd>=GetRowCount())
    {
        iRowEnd=GetRowCount()-1;
    }
    if(iColStart<0)
    {
        iColStart=0;
    }
    if(iColEnd==-1||iColEnd>=GetColumnCount())
    {
        iColEnd=GetColumnCount()-1;
    }

    COLORREF cFixedBackColor=GetFixedBkColor();
    COLORREF cBackColor=GetBkColor();
    ...
    for(i=iRowStart;i<=iRowEnd;i++)
    {
        for(j=iColStart;j<=iColEnd;j++)
        {
            ...
            if(bEditable)
            {
                ...
                pCell->crBkClr=cBackColor;
            }
            else
            {
                pCell->state|=GVIS_READONLY;
                pCell->crBkClr=cFixedBackColor;
            }
        }
    }
}
    ...
    do {
        ...
        while (pszCellText != NULL)
        {
            ...
            if (IsValid(TargetCell))
            {
				...
				/////////////////////////////////////
				// Added By FanHua, 2003.10.16
				// 用于通知父窗口编辑内容已经改变
				if(m_pMyParent)
				{
					m_pMyParent->PostMessage(GV_MODIFIED,TargetCell.row, TargetCell.col); //
				}
...
void CGridCtrl::ResetScrollBars()
{
    ...
    CRect rect;
    GetClientRect(rect);
    //rect.left  += GetFixedColumnWidth(); //removed by fanhua
    //rect.top   += GetFixedRowHeight(); //removed by fanhua
    //if (rect.left >= rect.right || rect.top >= rect.bottom) return;

   // CRect VisibleRect(GetFixedColumnWidth(), GetFixedRowHeight(), rect.right, rect.bottom);  //removed by fanhua
	CRect VisibleRect(0, 0, rect.right, rect.bottom); // changed by fanhua
    //CRect VirtualRect(GetFixedColumnWidth(), GetFixedRowHeight(), GetVirtualWidth(), GetVirtualHeight());
	CRect VirtualRect(0, 0, GetVirtualWidth(), GetVirtualHeight()); // changed by fanhua

    CCellRange visibleCells = GetUnobstructedNonFixedCellRange();
    //if (!IsValid(visibleCells)) return; // removed by fanhua


    if (VisibleRect.Height() < VirtualRect.Height())
        m_nVScrollMax = VirtualRect.Height()+120;//-1 ;//+ GetRowHeight(visibleCells.GetTopLeft().row); //removed by fanhua
    else
        m_nVScrollMax = 0;

    if (VisibleRect.Width() < VirtualRect.Width())
        m_nHScrollMax = VirtualRect.Width()+120;//-1 ;//+ GetColumnWidth(visibleCells.GetTopLeft().col); //removed by fanhua
    else
        m_nHScrollMax = 0;

    ASSERT(m_nVScrollMax < INT_MAX && m_nHScrollMax < INT_MAX); // This should be fine :)
    
//	m_nHScrollMax=GetVirtualWidth();	//	Added by fanhua
//	m_nVScrollMax=GetVirtualHeight();	//	Added by fanhua
    SCROLLINFO si;
	si.nMin	  =0;
    si.cbSize = sizeof(SCROLLINFO);
...
BOOL CGridCtrl::GetCellRangeRect(const CCellRange& cellRange, LPRECT lpRect) const
{
    ...
    lpRect->right  = MaxOrigin.x + GetColumnWidth(cellRange.GetMaxCol()/*-1 fanhua*/); 
                            ...
                            int nColumn  /* = -1 */)
{
	...
	SetSelectedRange(0,0,-1,-1); // Added By FanHua
	SetFocusCell(-1,-1);// Added By FanHua
...
void CGridCtrl::AutoSize()
{
	...
	//////////////////////////
	int i;
	for(i=0;i<m_arrayPics.GetCount ();i++)
	{
		CPicProperty &pic=m_arrayPics[i];
		SetRowHeight(pic.iRow  ,pic.iHeight );
	}
...
CImageList* CGridCtrl::CreateDragImage(CPoint *pHotSpot)
{
    ...
    *pHotSpot = rect.BottomRight(); 
    ...
    CDC MemDCSimple;
    CBitmap bm;
    MemDCSimple.CreateCompatibleDC(pDC);
    bm.CreateCompatibleBitmap(pDC, rect.Width(), rect.Height());
    CBitmap* pOldBitmap = MemDCSimple.SelectObject(&bm);
    MemDCSimple.SetWindowOrg(0,0);

    // Draw cell onto bitmap in memDC
    DrawCell(&MemDCSimple, cell.row, cell.col, rect, TRUE);

    // Clean up
    MemDCSimple.SelectObject(pOldBitmap);
    ReleaseDC(pDC);
...
CSize CGridCtrl::GetCellExtent(int nRow, int nCol, CDC* pDC)
{
    ...
    size += CSize(2*m_nMargin, m_nMargin);
...
BOOL CGridCtrl::DrawFixedCell(CDC* pDC, int nRow, int nCol, CRect rect, BOOL bEraseBk)
{
    ...
    if (!pDC->IsPrinting() && bEraseBk)
        pDC->FillSolidRect(rect, (Item.crBkClr == CLR_DEFAULT)? GetFixedBkColor() : Item.crBkClr);
...
BOOL CGridCtrl::DrawCell(CDC* pDC, int nRow, int nCol, CRect rect, BOOL bEraseBk)
{
    ...
    if (Item.state & GVIS_FOCUSED && !pDC->IsPrinting()) 
	{
        ...
        if (bEraseBk) pDC->FillSolidRect(rect, TextBkClr);
        rect.right--; rect.bottom--;    
        pDC->SelectStockObject(BLACK_PEN);
    ...
    // If The cell contains color, then draw the color
    // fanhua
    bool bSpecial=false;
    if(Item.szText.Left(1)=="&")
    {
        if(Item.szText.Mid(1,1)=="C")
        {
            bSpecial=true;
            CRect rect2=rect;
            rect2.DeflateRect(1,1,1,1);
            pDC->FillSolidRect(&rect2,RGB(0,0,0));
            rect2.DeflateRect(1,1,1,1);
            pDC->FillSolidRect(rect2,stovf(Item.szText.Mid(2)));
        }
    }

    if(!bSpecial)DrawText(pDC->m_hDC, Item.szText, -1, rect, Item.nFormat);
...
void CGridCtrl::OnEditCell(int nRow, int nCol, UINT nChar)
{
    ...
    if(IsCellCombo(nRow,nCol,listString))
    {
        // CInPlaceList auto-deletes itself   
        m_pInPlaceList=new CInPlaceList(this, rect, nRow, nCol, 
            listString, Item.szText, 
            nChar);
    }
    else
    {
...
void CGridCtrl::OnEndEditCell(int nRow, int nCol, CString str)
{
    ...
    CString strBak=GetItemText(nRow,nCol);
	SetItemText(nRow, nCol, str);

	if(m_pMyParent&&strBak!=str)
	{
		m_pMyParent->PostMessage(GV_MODIFIED,nRow,nCol); //
	}
...
void CGridCtrl::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	if(!IsSHIFTpressed())
	{		
		m_bShiftSelecting=false;
	}

	CWnd::OnKeyUp(nChar, nRepCnt, nFlags);
}

void CGridCtrl::SetFont(LOGFONT* lpLogFont )
{
	m_Font.DeleteObject ();
	m_Font.Detach ();
...
void  CGridCtrl::DrawPic(CString sPath, 
						 int iRow,
						 int iCol)
{

	CFHImage image;
	HRESULT h=image.Load (sPath );
	if(h!=S_OK)
	{
		return;
	}

	SetColumnWidth(iCol,m_iCellPicWidth);
	if(m_bCellPicKeepRatio)
	{
		int iHeight=(float)m_iCellPicWidth/image.GetWidth ()*image.GetHeight ();
		SetRowHeight(iRow ,iHeight);
		m_arrayPics.Add (CPicProperty(sPath,iRow,iCol,0,iHeight));
	}
	else
	{
		SetRowHeight(iRow,m_iCellPicHeight);
		m_arrayPics.Add (CPicProperty(sPath,iRow,iCol,0,m_iCellPicHeight));
	}
...
void  CGridCtrl::RemoveAllPics()
{
	m_arrayPics.RemoveAll ();
}

void   CGridCtrl::RemoveAllCombos()
{
    m_vComboContents.clear();
}

////////////////////////////////////////////////////
///
////////////////////////////////////////////////////
void  CGridCtrl::SetComboContent(int            iRow, 
                                 int            iCol, 
                                 CStringArray   &arrayString)
{
    CComboContent cc(iRow,iCol,arrayString);

    std::vector<CComboContent> ::iterator it;
    it=m_vComboContents.begin();
    while(it!=m_vComboContents.end())
    {
        if(*it==cc)
        {
            *it=cc;
...
/// Nov.17,2006
////////////////////////////////////////////////////
bool  CGridCtrl::IsCellCombo    (int            iRow, 
                                 int            iCol,
                                 CStringArray   &arrayString)
{
    std::vector<CComboContent> ::iterator it;
    ...
    while(it!=m_vComboContents.end())
    {
        if(it->m_iRow==-1)
        {
            if(iCol==it->m_iCol)
            {
                arrayString.Append(it->m_arrayContent);
                return true;
            }
        }
        ...
        {
            if(iRow==it->m_iRow&&iCol==it->m_iCol)
            {
                arrayString.Append(it->m_arrayContent);
                return true;
            }
...
void CGridCtrl::SetCellPicProperty(int iPicWidth,
								   int iPicHeight,
								   bool bPicKeepRatio,
								   bool bDrawPic)
{
	m_bCellPicKeepRatio		= bPicKeepRatio;
	m_iCellPicWidth			= iPicWidth;
	m_iCellPicHeight		= iPicHeight;
	m_bDrawCellPic			= bDrawPic;
}
void CGridCtrl::OnSelOKInCombo()
{
    if(!m_pInPlaceList)return;

    m_pMyParent->PostMessage(   GV_COMBOMODIFIED,
                                m_pInPlaceList->GetRow(),
                                m_pInPlaceList->GetCol());
}

void CGridCtrl::OnComboChanged(NMHDR* pNMHDR, LRESULT* pResult)
{
    OnSelOKInCombo();
}
